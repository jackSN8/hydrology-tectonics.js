...
float viscous = 0.0001;
float waterStart = 1;
PImage img; // To store the input image

float hmContrast = 1;//contrast higher when this high
...
void physicsTick2()
{
  ...
  {  
    ...
    {
    float angOffset = int(random(0,3))*0.5*PI;
    float [] deltas = new float [4];
    int startingInt = int(angOffset/(0.5*PI));

    for(int p=0; p<4; p++)
    {
        float direction = p*PI/2+angOffset;
        ////loop starting UD then clockwise (starting clockwise) 
        int xShift = int(sin(direction));//0,1,0,-1
...
void draw()
{
  ...
  {  
    for(int j=0; j<hght; j++)
    {
      ...
      //Heightmap coloring
      color chM = color(0,0,0);
      
      stroke(hM[i][j]*hmContrast,hM[i][j]*hmContrast,hM[i][j]*hmContrast);
      point(i+wdth,j);
    }
  }
}
